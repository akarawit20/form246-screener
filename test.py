...
for trader in active_traders: #active_form246.list_trader():
    ...
    for date, ticker, type in tqdm(transactions.to_records()):
        try:
            transaction = get_quote(ticker, date, 30)
            if type == 'Acquisition':
                transaction.plot(label=ticker, color='green')
                avg = np.average(transaction)
                purchase_slope.append(avg)
            else: #Disposition
                transaction.plot(label=ticker, color='red')
                avg = np.average(transaction)
                sale_slope.append(avg)
        except:
            pass
    performance = [trader, np.average(purchase_slope), np.average(sale_slope)]
